...
<!-- //////////////////////////////////////////// -->
 ...
 <script src="lib/leaflet/bundle.js"></script>  
  <script src="data/DELEG_VFF.geojson"> </script>
     ...
     clear: both;
      ...
      <body>
	     ...
	     <div class="row" style="position:relative; left:200px; top:-50px; bottom:0px; width:1100px;  display: block; margin: 0 auto; font-size: 12px; float: center;" >
			 ...
			 <form method="get" action="index_GOV_V.php"> <div class="column"><button type="submit"  id="Btn2" class="btnStyle span3">GOUVERNOURATS</button></div></form>
		    ...
		    // les variables
			 ...
			 var markers = new Array();
               ...
               function resetHighlight(e){
			      DelegLayer.resetStyle(e.target);
			    }
				...
				function DelegOnEachFeature(feature, layer){
				
				
				  layer.bindPopup(
				(('<b>Delegation: &nbsp')+(feature.properties.MJ__Delegations))+'</b><br>'+
				   (('-Nombre de population:&nbsp')+(feature.properties.MJ__Population))+'<br>'+
				   (('-Nombre des Menages:&nbsp')+(feature.properties.MJ__Menages))+'<br>'+
				   (('-Nombre des Logements:&nbsp')+(feature.properties.MJ__Logements))+'<br>'
				
				);
				   
				   //layer.bindLabel(
				   //(('Delegation: &nbsp')+(feature.properties.MJ__Delegations))+'<br>'+
				   //(('Nbr de population:&nbsp')+(feature.properties.MJ__Population))+'<br>'+
				   //(('Nbr des Menages:&nbsp')+(feature.properties.MJ__Menages))+'<br>'+
				   //(('Nbr des Logements:&nbsp')+(feature.properties.MJ__Logements))+'<br>'
				   //,{noHide : true});
				 ...
				 markers[markersCount - 1].bindLabel(
				   (feature.properties.MJ__Delegations),
				   {noHide : true,
				     className:'map-label',
					 pane: 'mapPane'
					 });
			   ...
			   // population style
			   function getPopColor(MJ__Population){
			     if (MJ__Population > 80000){
				     return '#BB0C01';
				 } else if (MJ__Population > 50000){
                     return '#EB1616';				 
			     } else if (MJ__Population > 30000){
                     return '#FF6400';				 
			     } else if (MJ__Population > 20000){
                     return '#FF9E00';				 
			     } else if (MJ__Population > 10000){
                     return '#FFD100';				 
			     } else if (MJ__Population < 10000){
                     return '#F8FC72';	
			     }
               ...
               function delegStayle(feature){
	              return {
		            fillColor: getPopColor(feature.properties.MJ__Population),
			        weight : 2,
			        opacity : 1,
			        color: 'white',
			        dashArray : 3,
				  ...
				  var Google_Earth = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{
                    ...
                    var Blanc = L.tileLayer(imageUrl,{maxZoom: 17, minZoom: 6}).addTo(map);
                     ...
                     // Import des données vecteur GeoJISON
                      ...
                      DelegLayer = L.geoJson(
                      DELEG_VFF,
	                    {
					     style : delegStayle,
						 onEachFeature: DelegOnEachFeature
					     }
                       ).addTo(map);					   
				   ...
				   var legend = L.control({position : 'bottomright'});
				     legend.onAdd= function(map){
						 ...
						 var labels =[
						 "Population Superieur a 80 000",
						 "Population Superieur a 50 000",
						 "Population Superieur a 30 000",
						 "Population Superieur a 20 000",
						 "Population Superieur a 10 000",
						 "Population Inferieur a 10 000"
						 ];
						 var grades = [80001, 50001, 30001, 20001, 10001, 9000];
