...
<script src="lib/leaflet/bundle.js.map"></script>  
<script src="data/DELEG02.geojson"> </script>
...
<!-- //////////////////////////////////////////// -->
 ...
 <script src="lib/leaflet/bundle.js"></script>   
  <script src="data/GOV_VF.geojson"> </script>
      ...
      <body>
  			...
  			<div class="container">
  				...
  				<div class="navbar-header">
  					...
  					<a class="navbar-brand" href="http://www.graphicom.tn/index.html">Graphicom</a>
  				</div>
	     ...
	     <div class="row" style="position:relative; left:200px; top:-50px; bottom:0px; width:1100px;  display: block; margin: 0 auto; font-size: 12px; float: center;" >
		 
		     <div style=" top:5px;">
             <form method="get" action="index_REGION_V.php"><div class="column"><button type="submit" href="index_GOV.php"   id="Btn1" class="btnStyle span3">REGIONS</button></div></form>
			  </div>
			 <form method="get" action="index_DELEG_V.php"> <div class="column"><button type="submit"  id="Btn2" class="btnStyle span3">DELEGATIONS</button></div></form>
		    ...
		    // les variables
			 ...
			 var markers = new Array();
               ...
               function resetHighlight(e){
				...
				function GovOnEachFeature(feature, layer){
				   
				   layer.bindPopup(
				(('<b>Governorat: &nbsp')+(feature.properties.VFG__Gover))+'</b><br>'+ 
				   (('-Nombre de population:&nbsp')+(feature.properties.VFG__Popul))+'<br>'+
				   (('-Nombre des Menages:&nbsp')+(feature.properties.VFG__Menag))+'<br>'+
				   (('-Nombre des Logements:&nbsp')+(feature.properties.VFG__Logem))+'<br>'+
				   (('-Depense annuel par personne en dinar:&nbsp')+(feature.properties.VFG__Depense))+'<br>'
				
				);
				   
				   
				   //layer.bindLabel(
				   //(('Governorat: &nbsp')+(feature.properties.VFG__Gover))+'<br>'+
				   //(('Nbr de population:&nbsp')+(feature.properties.VFG__Popul))+'<br>'+
				   //(('Nbr des Menages:&nbsp')+(feature.properties.VFG__Menag))+'<br>'+
				   //(('Nbr des Logements:&nbsp')+(feature.properties.VFG__Logem))+'<br>'+
				   //(('Depense annuel par personne en dinar:&nbsp')+(feature.properties.VFG__Depense))+'<br>'
				   //,{noHide : true});
			   ...
			   // population style
			   function getPopColor(VFG__Popul){
			     if (VFG__Popul > 900000){
				     return '#BB0C01';
				 } else if (VFG__Popul > 600000){
                     return '#EB1616';				 
			     } else if (VFG__Popul > 500000){
                     return '#FF6400';				 
			     } else if (VFG__Popul > 400000){
                     return '#FF9E00';				 
			     } else if (VFG__Popul > 200000){
                     return '#FFD100';				 
			     } else if (VFG__Popul < 200000){
                     return '#F8FC72';	
			     }
               ...
               function GovStayle(feature){
	              return {
		            fillColor: getPopColor(feature.properties.VFG__Popul),
			        weight : 2,
			        opacity : 1,
			        color: 'white',
			        dashArray : 3,
				  ...
				  var Google_Earth = L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{
                    ...
                    var Blanc = L.tileLayer(imageUrl,{maxZoom: 17, minZoom: 6}).addTo(map);
                    ...
                    // Import des données vecteur GeoJISON
                      ...
                      GOV_VF,
	                    {
					     style : GovStayle,
						 onEachFeature: GovOnEachFeature
					     }
                       ).addTo(map);
				   ...
				   var legend = L.control({position : 'bottomright'});
				     legend.onAdd= function(map){
						 ...
						 var labels =[
						 "Population Superieur a 900 000",
						 "Population Superieur a 600 000",
						 "Population Superieur a 500 000",
						 "Population Superieur a 400 000",
						 "Population Superieur a 200 000",
						 "Population Inferieur a 200 000"
						 ];
						 var grades = [900001, 600001, 500001, 400001, 200001, 190000];
